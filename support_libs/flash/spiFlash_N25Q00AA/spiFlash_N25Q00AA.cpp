...
int32_t SPIFlash_N25Q00AA::init() {
	...
	cs.init(true, 1, 1);
        ...
        /* The status register must be read once after write operations. This is
         * done implicitly before all following operations via waitForFlash,
         * except after a soft reset of the microcontroller. Thus, the status
         * register is first read in order for the initialization to work.
         */
        uint8_t result[2];
	uint8_t send[3];
	send[0] = CMD_RDFSR;
	...
	waitForFlash(); //< still needed to allow re-initialization
